...
if "\n\n" in lines:
    rules, msgs = [line for line in lines.split("\n\n")]
    msgs = [msg for msg in msgs.split("\n")]
else:
    rules = [rule.split(":") for rule in lines.split("\n")]
    rules = [[subRule[0], subRule[1].split("|")] for subRule in rules]
    rules = [
        [subRule[0], [rule.strip().split(" ") for rule in subRule[1]]]
